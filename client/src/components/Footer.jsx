...
const Footer = () => {
  ...
  <div className="flex items-end gap-3 px-4 sm:px-6 lg:px-10 py-6 w-max">
    {ImageArray.map((val, idx) => (
      <div
        key={idx}
        className="flex-shrink-0 overflow-hidden rounded-t-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 group min-w-[80px]"
      >
        <img
          src={val.imgIcon}
          alt={`Gallery image ${idx + 1}`}
          width={80}
          className={`${val.height} w-110 object-cover transition-transform duration-300 group-hover:scale-110`}
        />
      </div>
    ))}
  </div>
    ...
    <footer className="bg-gradient-to-b from-[#00457C] to-[#003461] text-white relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-transparent to-[#00457C] z-10"></div>
